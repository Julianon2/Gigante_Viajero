...
// ================================================================
// Parche para que authAPI reconozca sesiones iniciadas con Google

const _originalCheckAuth = SessionManager.checkAuth.bind(SessionManager);

SessionManager.checkAuth = function() {
    // Primero intentar con authAPI normal
    const normalAuth = _originalCheckAuth();
    if (normalAuth) return true;

    // Si falló, verificar si hay sesión de Google en localStorage
    try {
        const googleUser = localStorage.getItem("gigante_user");
        const googleToken = localStorage.getItem("gigante_token");
        const googleAuth = localStorage.getItem("gigante_auth");

        if (googleUser && googleToken && googleAuth === "true") {
            const user = JSON.parse(googleUser);
            if (user && user.email && user.provider === "google") {
                this.log(`Sesión Google detectada: ${user.email}`, 'success');
                return true;
            }
        }
    } catch (e) {
        this.log('Error verificando sesión Google: ' + e.message, 'error');
    }
